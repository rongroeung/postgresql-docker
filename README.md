...
#### >>> Open `pgAdmin` > `crwebdb` > `Schemas` > `Tables` > Right click on each table > `Restore...` > Choose Filename `crwebdb_20250919.sql` > `Restore`
#### >>> Note: No need to do step2 (Create Tables) & step3 (Create Insert Functions) as the backup file already contains database schema and all data.
